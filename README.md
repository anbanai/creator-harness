...
视频生成和视频复刻仅通过 Studio 下发托管任务，并使用各自独立的官方运行镜像；Agent 名称分别为 `montage` 和 `hypit`。需要先启用服务端能力并准备官方 Runtime Profile。仅安装插件不包含浏览器、官方引擎或 Provider 配置，本地直接调用不会启动生产视频工作流。
...
视频生成使用平台托管的固定版本 OpenMontage 运行镜像，通过 Studio 项目、任务和计划执行，交付 `output/final.mp4`、封面、`output/montage-project.json` 与 `output/delivery-manifest.json`。视频复刻使用平台托管的固定版本官方 Hypit 运行镜像，交付 `output/final.mp4`、封面、`output/project.json`、`output/project.zip` 和检查报告；再次改编复用完整工程与 Results。平台配置官方 Runtime Profile 与 Provider，Agent 直接读取镜像内同版本的官方 Skill。相关功能默认关闭，先进行内部验证；对外托管前需取得上游要求的商业授权。
