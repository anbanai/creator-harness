...
DSH is not a separate Anban business workflow or Skill tree. `plugins/skills/**`
...
## 4.1.14 release-operator checklist
...
1. **public npm package (primary)**: an exact stable SemVer specifier such as
   `@anban/dsh-plugin@4.1.14`; do not use an unversioned package, dist-tag, or
   version range.
...
`royalmorty/anbanwriter` GitHub Release. Verify before installation:
...
`file:/absolute/path/anban-dsh-plugin-4.1.14.tgz`; a `file:` directory and an
...
```bash
pnpm pack --json
PACKED_TARBALL="/absolute/path/anban-dsh-plugin-4.1.14.tgz"
