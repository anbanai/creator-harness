...
## 4.1.17 release-operator checklist
...
1. **public npm package (primary)**: an exact stable SemVer specifier such as
   `@anban/dsh-plugin@4.1.17`; do not use an unversioned package, dist-tag, or
   version range.
...
`file:/absolute/path/anban-dsh-plugin-4.1.17.tgz`; a `file:` directory and an
...
```bash
pnpm pack --json
PACKED_TARBALL="/absolute/path/anban-dsh-plugin-4.1.17.tgz"
