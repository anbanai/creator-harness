...
> **关键规则**：封面、内容图和尾图均不预设是否使用任务上传图片。每页根据 `image-plan.md` 独立选择 0、1 或多张任务原图；没有相关任务参考时使用纯文生图。项目风格图只使用分析得到的文本风格块，原图路径不得进入生成调用。每张内容图必须使用对应信息点构造独立 prompt；共享“风格延续：{style}”文本块只负责调性一致，不能替代对产品事实和参考素材用途的逐页判断。
