...
当且仅当托管 user message 含 Server 注入的“发布恢复模式”且明确要求“从 `image_generation` 阶段继续”时，执行下面的恢复分支，跳过常规十步流程的步骤 1 至 6c：

1. 读取并复用 Server 已恢复的 `output/04-article-final.md`、`seo-result.md`、`visual-rhythm-plan.md`、封面/配图计划、`05-article.html`、审核文件和 `draft.json`；这些文件缺失或损坏时写结构化失败诊断，不得重新创作替代。
2. 不得重新执行选题、正文创作、SEO 或语义审核，不得改写标题、摘要、正文、模板选择和既有视觉策略。
3. 只按图片开关和既有计划重新生成缺失或失败的封面/正文图片，完成必要的图片审核与上传；已成功且可验证的图片不得重复生成。
4. 将新图片 URL 回填既有 `layout_plan`，从步骤 8 重新渲染最终 HTML；只更新受新图片影响的视觉验收记录，并重新生成符合 1.0 合约的 `output/draft.json`。
5. 托管进度仍创建 research、writing、delivery 三个 Task；校验恢复文件后立即把 research、writing 标记完成，只在 delivery 中执行视觉恢复、HTML 更新和发布包交付。
6. 不调用任何草稿发布 MCP。Server 在本次恢复执行终态重新校验并完成公众号投递。
...
- 3 次仍未通过 → 标记 `quality_status=failed`，继续生成后续 slot，但不得把失败图用于发布
...
> **图片开关前置**：下列「封面图必须成功生成」「配图内容贴切」「独立视觉审核闭环」「参考链一致」「图文并茂」「视觉多样性」六项硬性要求，**仅在对应图片开关开启时生效**；开关关闭时跳过且不计为失败（见「图片生成模式」）。
...
> **图片开关前置**：下列涉及封面/配图产物的勾选项，在对应图片开关关闭时不适用、跳过判定（见「图片生成模式」）；纯文字文章时草稿字段不含 `thumb_media_id`。
...
- `get_project_profile` 调用成功但缺少可选语义配置时，采用并记录 Agent 默认值后继续
- `get_project_profile` 或其他必需 MCP 调用不可用、报错或无有效响应时，按 `article_mcp_call_failed` 写结构化失败态并结束；不得把调用失败解释成配置缺失
...
- 质量报告统一保存在 `output/context-brief.md`、`output/content-quality-report.md`、`output/final-review.md`
...
4. **透明记录**：把决策、警告和失败原因写入任务产物或进度记录，不向用户提问或等待选择
5. **尊重配置**：遵循托管上下文与 profile 配置；调用失败严格按「MCP 工具使用规则」处理
