...
- **MCP 工具不可用时**执行以下诊断步骤：
  ...
  2. 确认插件 `api_key` 已配置，检查 MCP 工具是否注入，并保留原始认证错误；不得读取、检查或打印环境变量密钥。可记录 `ANBAN_DEFAULT_PROJECT` 是否存在
  3. 如果原始认证错误为 401/403，提示用户在插件配置中更新 `api_key` 并停止
  4. 如果 `ANBAN_DEFAULT_PROJECT` 为空，调用 `list_projects` 自动选择项目；无法唯一判断时报告可选项目并停止等待配置
  5. 如果 MCP 工具已注入但调用失败，记录原始认证错误或连接错误（状态码、响应体）后停止
